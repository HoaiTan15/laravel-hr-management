...
<main class="stitch-dashboard">
    <div class="stitch-page">
        <div class="stitch-context-row"><div class="stitch-context"><span class="stitch-kicker">Hệ thống nhân sự</span><span class="stitch-context-slash">/</span><span>Không gian làm việc cá nhân</span></div><div class="stitch-date"><span></span> Hôm nay: {{ $weekdayLabels[now()->dayOfWeek] }}, {{ now()->format('d/m/Y') }}</div></div>

        <section class="stitch-summary-grid">
            <article class="stitch-summary-card"><div class="stitch-summary-head"><span>Trạng thái hôm nay</span><div class="stitch-summary-icon primary"><span class="material-symbols-outlined">badge</span></div></div><div class="stitch-summary-value stitch-status-value">@if($attendance?->check_out_at)<span class="stitch-status-pill completed">Đã Check-out</span>@elseif($attendance?->check_in_at)<span class="stitch-status-pill working">Đang làm việc</span>@else<span class="stitch-status-pill pending">Chưa Check-in</span>@endif</div><span class="stitch-summary-caption">Theo dõi ca làm việc hôm nay</span></article>
            <article class="stitch-summary-card"><div class="stitch-summary-head"><span>Giờ Check-in</span><div class="stitch-summary-icon primary"><span class="material-symbols-outlined">login</span></div></div><div class="stitch-summary-value">{{ $attendance?->check_in_at?->format('H:i') ?? '—' }} <small>{{ $attendance?->check_in_at ? 'Hôm nay' : 'Chưa ghi nhận' }}</small></div><span class="stitch-summary-caption"><span class="material-symbols-outlined">verified</span> Dữ liệu chấm công cá nhân</span></article>
            <article class="stitch-summary-card"><div class="stitch-summary-head"><span>Công việc hôm nay</span><div class="stitch-summary-icon secondary"><span class="material-symbols-outlined">assignment_turned_in</span></div></div><div class="stitch-summary-value">{{ $taskCount }} <small>công việc</small></div><span class="stitch-summary-tag">{{ $tasksDueToday }} việc đến hạn hôm nay</span></article>
            <article class="stitch-summary-card"><div class="stitch-summary-head"><span>Phiếu đang xử lý</span><div class="stitch-summary-icon primary"><span class="material-symbols-outlined">pending_actions</span></div></div><div class="stitch-summary-value">{{ $pendingRequestCount }} <small>phiếu</small></div><span class="stitch-summary-caption">PYC của riêng bạn</span></article>
        </section>

        <div class="stitch-content-grid">
            <div class="stitch-main-column">
                <section class="stitch-panel">
                    <div class="stitch-panel-head"><div class="stitch-panel-title"><div class="stitch-panel-icon primary"><span class="material-symbols-outlined">checklist</span></div><div><h2>Công việc hôm nay</h2><p>Nhiệm vụ chuyên môn được giao gần đây</p></div></div><span class="stitch-count-pill">{{ $tasks->count() }} mục</span></div>
